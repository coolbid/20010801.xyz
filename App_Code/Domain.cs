...
namespace domain
{
    public class domainname : System.Web.UI.Page
    {
        public BasicPage bp = new BasicPage();
        public static string strconfig = HttpRuntime.AppDomainAppPath.ToString() + "/upload/config/site.config";
        /// <summary>
        /// 限制域名访问
        /// </summary>
        ...
        /// 判断购买功能权限
        /// </summary>
        public void GetBuyControl()
        {
            restrict();
            string BuyControl = GetIndexConfigValue("BuyControl");
            if (BuyControl != "1")
            {
                System.Web.HttpContext.Current.Response.Write("<script>alert('您的权限不足，无法打开该功能页面。请联系管理员！');window.location.href='/home'</script>");
            }
