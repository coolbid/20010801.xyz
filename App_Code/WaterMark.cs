...
namespace Basic.Tools
{
    ...
    {
        /// <summary>
        /// 图片水印{服务器图片相对路径，保存文件名，水印文件相对路径，图片水印位置 0=不使用 1=左上 2=中上 3=右上 4=左中  9=右下，附加水印图片质量0--100，水印的透明度 1--10 10为不透明}
        /// </summary>
        ...
        public static void AddImageSignPic(string imgPath, string filename, string watermarkFilename, int watermarkStatus, int quality, int watermarkTransparency)
        {
            ...
            filename = Utils.GetMapPath(filename);//D:\\企智网络\\400301.com\\upload\\image\\20150321\\201503211135115468.png
            ...
            if (!File.Exists(watermarkFilename))//D:\\企智网络\\400301.com\\400301_Admin\\watermark.png
                return;
            ...
            {
                int _w = img.Width / watermark.Width;
                int _h = img.Height / watermark.Height;

                for (int i = 0; i < _w + 1; i++)
                {
                    for (int j = 0; j < _h + 1; j++)
                    {
                        g.DrawImage(watermark, new Rectangle(watermark.Width * i, watermark.Height * j, watermark.Width, watermark.Height), 0, 0, watermark.Width, watermark.Height, GraphicsUnit.Pixel, imageAttributes);
                    }
                }
            }
            else
